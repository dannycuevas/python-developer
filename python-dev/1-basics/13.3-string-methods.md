...
- We can have a String, and use `split()` to convert that String of whatever group of "words" to a List of words - thus, turning 1 object (String) into many objects (a list)
- Notice the List with commas and quotes for every item
```python
"Demons do not sleep, overcome them".split()

>>>['Demons', 'do', 'not', 'sleep,', 'overcome', 'them']
```

### Methods: "upper" VS "capitalize"
🔤 `.upper()` vs `.capitalize()`

🟢 `.upper()` is equal to:
👉 Turns ALL letters into uppercase

🟢 `.capitalize()` is equal to:
👉 Only capitalizes the "First" letter and makes the rest lowercase
